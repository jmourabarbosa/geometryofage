...
plot_gamm <- function(model,predictor = 'age', x_axis_label = '', y_axis_label = '', sig = TRUE, color_map = NULL) {
 ...
 if (!is.null(color_map)) {
  ...
  gammplot <- ggplot(dat, aes_string(x = predictor, y = outcome, group = "ID")) + 
    # geom_line(alpha = .4) + 
...
dat <- read.csv('rotation_odrd_cue_dis_500boot_15group_30unit_popu_mean.csv', 
...
# plot boxes of raw data
```{r}
# Divide the "mature" column into 15 intervals
dat$mature_interval <- cut(dat$mature, breaks = 15, labels = FALSE)

# Calculate the mean "mature" value for each interval
interval_means <- aggregate(dat$mature, by = list(dat$mature_interval), FUN = mean)

# Rename columns for clarity
names(interval_means) <- c("Interval", "MeanMature")

# Merge interval means back into the original data for plotting
dat <- merge(dat, interval_means, by.x = "mature_interval", by.y = "Interval")

# Add a discrete grouping variable for the boxplot
dat$group <- as.factor(dat$mature_interval)

# Plotting
current_plot <- ggplot(dat, aes(x = mature, y = rotation, group = MeanMature)) +
  geom_boxplot(outlier.colour="red", outlier.alpha = 0.1, notch=TRUE) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))


 pdf_file_name <- paste0('boxplot_rotation.pdf')
 # Save the plot in PDF format
  ggsave(pdf_file_name, plot = current_plot, device = 'pdf', width = 6, height = 6)
...
# Assuming 'data' is your dataframe with 'mature' and 'rotation' columns
ggplot(dat, aes(x = mature, y = rotation, group = MeanMature)) +
  geom_boxplot(outlier.colour="red", outlier.alpha = 0.1, notch=TRUE) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) 
