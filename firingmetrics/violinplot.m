...
elseif isnumeric(data) % numeric input   
    ...
    if hascategories && numel(data) == numel(cats)    
        ...
        for n = 1:length(catnames)
            ...
            % violins(n) = Violin({thisData}, double(string(thisCat)), varargin{:}); % JZ, to plot at any x position
             violins(n) = Violin({thisData}, n, varargin{:});
        end
        set(gca, 'XTick', 1:length(catnames), 'XTickLabels', catnames_labels);
        % set(gca, 'XTick', double(string(catnames)), 'XTickLabels', catnames_labels);% JZ, to label at any x position
        set(gca,'Box','on');
        return
    else
        data = {data};
