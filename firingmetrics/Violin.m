classdef Violin < handle
    ...
    methods
        function obj = Violin(data, pos, varargin)
            ...
            if length(density) > 1
               jitterstrength = interp1(unique(value), density*width, data, 'linear','extrap');
    ...
    methods (Static)
        function [density, value, width] = calcKernelDensity(data, bandwidth, width)
            ...
            value(1) = min(data);
            value(end) = max(data);
            value = [value(1)*(1-1E-5)-(1E-5), value, value(end)*(1+1E-5)+(1E-5)]; % JZ 20231016
            density = [0, density, 0];
