...
dat <- read.csv('rd_contra_and_bi.csv', 
...
for(model_name in names(models_mat_list)) {
  ...
  ymin_value <- y_range[1] - 0.01 * (y_range[2] - y_range[1])
  ymax_value <- y_range[1] 
   formatted_model_name <- gsub("_", " ", model_name)
  current_plot <- current_plot + labs(title = formatted_model_name) +
    annotate(geom = "rect", 
             xmin = dev_change[1], 
             xmax = dev_change[2], 
             ymin = ymin_value, 
...
current_plot <- ggplot(combined_df, aes(xmin = start,
  ...
  labs(x = "Maturation age", y = "Tracts", title = "RD") +
  theme_minimal() +
...
generate_plot <- function(data, title_suffix) {
  ...
  ggplot(data, aes(xmin = start, xmax = end,
    ...
    scale_fill_gradient2(low = "darkblue", mid = "white", high = "red", midpoint = 0,
                         name = "% change per month",
                         limits = range(c(-2.0, 1.5), na.rm = TRUE)) +
    labs(x = "Maturation age", y = "Tracts", title = paste("RD", title_suffix)) +
    theme_minimal() +
    theme(axis.text.y = element_text(size = 10, hjust = 1),
...
```{r}
  # Define file names based on the model name
  png_file_name1 <- paste0('RD_timing1.png')
  pdf_file_name1 <- paste0('RD_timing1.pdf')
