%% load data
clearvars
% load('odrd_data_both_sig_20230927.mat');
load('odrd_data_sig_small_rf_20231018.mat');
try
    odrd_data = odrd_data_new;
catch
end
try
    select_nan = find(neuron_info.best_cue~=0);
    neuron_info = neuron_info(select_nan,:);
    odrd_data = odrd_data(select_nan,:);
catch
end
%% seg data; optional
...
for g = 1:size(groups)
    ...
    for n = 1:length(Neurons)
        ...
        for nt = 1: size(curr_binned_data,2)
            omega_result = mes1way(curr_binned_data,'omega2','group',stim_neuron');
            PEV(g).group(n).omega2(nt) = omega_result.omega2;
            PEV(g).group(n).ci_lo(nt) = omega_result.omega2Ci(1);
            PEV(g).group(n).ci_hi(nt) = omega_result.omega2Ci(2);
        end
    end
    disp(g)
end
...
epoch_start = 0; % in s
epoch_end = 0.5; % in s
neuron_index = 1:size(odrd_data,1);
PEV = table;
Neurons = neuron_info.Neurons;
group_data = odrd_data;
for n = 1:length(Neurons)
    neuron_data = group_data(n,:);
    rates_neuron = [];
    rates_neuron1 = [];
    ...
    if size(stim_neuron,1)>=64 & sum(rates_neuron)>0
        for nb = 1%:10
        ...
        % PEV.ci(n,:) = omega_result.omega2Ci;
    else
        PEV.omega2(n) = nan;
    end
