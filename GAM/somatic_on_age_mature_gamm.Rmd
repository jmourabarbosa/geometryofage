...
dat <- read.csv('somatic.csv', 
                stringsAsFactors = FALSE) 
```
# clean data
```{r}
header_list <- as.list(names(dat))
names_to_omit <- c("age","mature","ID","sex","DOB","DOM","DATE")
...
# normalize
```{r}
# List of outcomes to normalize
outcomes_to_normalize <- unlist(list_all)

# Normalize specified outcomes to their individual maximum
dat_normalized <- dat %>%
  group_by(ID) %>%
  mutate_at(vars(one_of(outcomes_to_normalize)), ~ . / max(., na.rm = TRUE) * 100) %>%
  ungroup()
...
# inspection
```{r}
# example_model <- models_age_list[["FA_MTG_L_age"]]
# 
# # Check the summary of the model
# print(summary(example_model$gam))
# print(summary(example_model$lme))
# 
# # Try plotting this model
# example_plot <- plot_gamm(
#   predictor = 'age',
#   model = example_model, 
