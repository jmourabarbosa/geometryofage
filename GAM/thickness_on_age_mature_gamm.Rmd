...
dat <- read.csv('thickness_Charm1_contraonly.csv', 
...
# listregions_name =  c('lobes_thickness', 'parietal_thickness', 'frontal_thickness', 'temporal_thickness', 'occipital_thickness', 'all_thickness')
```
...
for(model_name in names(models_mat_list)) {
  ...
  theme(axis.text.y = element_text(size = 5, hjust = 1),
    ...
    labs(x = "Maturation age", y = "Thickness")
  print(current_plot)
  # Define file names based on the model name
  png_file_name <- paste0(model_name, '.png')
  pdf_file_name <- paste0(model_name, '.pdf')
...
write.csv(summary_table, "thickness_charm2_summary_table.csv", row.names = FALSE)
```
...
```{r}
# Define file names based on the model name
png_file_name1 <- paste0('thickness_contra_timing.png')
...
create_plot <- function(data) {
  ggplot(data) + 
    ...
    labs(x = 'Mid-adolescence age (mon.)', y = 'Thickness') +
    theme_bw() +
    theme(
...
ggsave('thickness_Charm1_combined_plot_sig_epochs.png', plot = combined_plot, device = 'png', width = 12, height = 6, dpi = 300)
ggsave('thickness_Charm1_combined_plot_sig_epochs.pdf', plot = combined_plot, device = 'pdf', width = 12, height = 6)
...
write.csv(percentage_change_df, file = "percentage_change_overtime_thickness_charm2.csv", row.names = TRUE)
```