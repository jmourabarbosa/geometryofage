...
dat <- read.csv('performance_odrd.csv', 
                stringsAsFactors = FALSE) 
# dat <- read.csv('performance_odr.csv', 
#                 stringsAsFactors = FALSE) 
```
# clean data
```{r}
header_list <- as.list(names(dat))
names_to_omit <- c("percorr")
...
for(L in 1:length(list_of_regressor)){
  ...
  for(R in 1:length(list_all)){
    ...
    for(depend in current_somatic){
      print(paste("Regressor:", regressor, "| Response:", depend))
      
      # Define a formula for the GAMM to model the current response as a smoothed function of the current regressor
      model_formula <- as.formula(paste(depend, "~ s(", regressor, ", k = 5, fx = FALSE, bs = 'tp')"))
...
for(L in 1:length(list_of_regressor)){
  ...
  for(R in 1:length(list_all)){
    current_depend = list_all[[R]]

    for(depend in current_depend){
      print(paste("Regressor:", regressor, "| Response:", depend))
      
      # Define a formula for the GAMM to model the current response as a smoothed function of the current regressor
      model_formula <- as.formula(paste(depend, "~ s(", regressor, ", k = 20, fx = FALSE, bs = 'tp')"))
