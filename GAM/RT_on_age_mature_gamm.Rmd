...
dat <- read.csv('sac_RT.csv', 
                stringsAsFactors = FALSE) 
```
#read odrd data
```{r}
# Read the dataset from a CSV file
dat <- read.csv('sac_odrd.csv', 
                stringsAsFactors = FALSE) 
...
model_formula <- as.formula("RT ~ s(age, k = 5, fx = FALSE, bs = 'cs')")
# Fit the GAMM with the formula, and a random effect of 'id' on 'age'
gamm_rt_on_age <- gamm(as.formula(model_formula), 
                                      #family = 'quasibinomial',
                                      random = list(ID=~1+age),
                                      data = dat %>% drop_na(all_of('RT')),
                                      method = 'REML')
# View summary statistics of the GAM and linear mixed-effects components of the model
summary(gamm_rt_on_age$gam)
summary(gamm_rt_on_age$lme)
# Check diagnostics of the GAM model
par(mfrow = c(2,2))
gam.check(gamm_rt_on_age$gam)
```
# model RT on mature
```{r}
# Define a formula for the GAMM to model 'rate' as a smoothed function of mature ('mature')
model_formula <- as.formula("RT ~ s(mature, k = 5, fx = FALSE, bs = 'cs')")
# Fit the GAMM with the formula, and a random effect of 'id' on 'mature'
gamm_rt_on_mat <- gamm(as.formula(model_formula), 
                                      family = 'gaussian',
                                      random = list(ID=~1+mature),
                                      data = dat %>% drop_na(all_of('RT')), 
                                      method = 'REML')
# View summary statistics of the GAM and linear mixed-effects components of the model
summary(gamm_rt_on_mat$gam)
summary(gamm_rt_on_mat$lme)
# Check diagnostics of the GAM model
par(mfrow = c(2,2))
gam.check(gamm_rt_on_mat$gam)
```
# find sig range
```{r}
# Identify significant periods of developmental change
rt_on_age_devchange <- calc_dev_change(gamm_rt_on_age)
rt_on_age_devchange
rt_on_mat_devchange <- calc_dev_change(gamm_rt_on_mat)
rt_on_mat_devchange
```
...
rt_on_age_plot <- plot_gamm(
  model = gamm_rt_on_age, 
  predictor = 'age',
  x_axis_lab = 'Age (month)',
  y_axis_lab = 'Reaction time (s)',
  sig = TRUE, color_map = my_color)

# Extract y-axis range from current plot
  y_range <- layer_scales(rt_on_age_plot)$y$range$range
  # Set ymin and ymax based on y_range.
  ymin_value <- y_range[1] - 0.001 * (y_range[2] - y_range[1])
  ymax_value <- y_range[1] 
# Annotate the plot with a bar indicating significant developmental change
rt_on_age_plot <- rt_on_age_plot +
  annotate(geom = "rect", 
           xmin = rt_on_age_devchange[1], 
           xmax = rt_on_age_devchange[2], 
           ymin = ymin_value, 
           ymax = ymax_value, 
           fill = '#476FD1') +
...
```{r}
# Plot the model results using previously defined function
rt_on_mat_plot <- plot_gamm(
  model = gamm_rt_on_mat, 
  predictor = 'mature',
  x_axis_lab = 'Maturation (month)',
  y_axis_lab = 'Reaction time (s)',
  sig = TRUE, color_map = my_color)
# Extract y-axis range from current plot
  y_range <- layer_scales(rt_on_mat_plot)$y$range$range
  # Set ymin and ymax based on y_range
  ymin_value <- y_range[1] - 0.001 * (y_range[2] - y_range[1])
  ymax_value <- y_range[1] 
# Annotate the plot with a bar indicating significant developmental change
rt_on_mat_plot <- rt_on_mat_plot + 
  annotate(geom = "rect", 
           xmin = rt_on_mat_devchange[1], 
           xmax = rt_on_mat_devchange[2], 
           ymin = ymin_value, 
           ymax = ymax_value, 
           fill = '#476FD1') +
...
gridExtra::grid.arrange(rt_on_age_plot, rt_on_mat_plot,
                        ncol = 2)
combined_plot <- gridExtra::arrangeGrob(rt_on_age_plot, rt_on_mat_plot,
                                        ncol = 2,
                                        top = "Predicted Variable: RT")
```
#save plots
```{r}
# Save the current grid display
ggsave("RT_odrd.png", plot = combined_plot, width = 10, height = 6)
ggsave("RT_odrd.pdf", plot = combined_plot, width = 10, height = 6)
```

# save fitted curve
```{r}
# Assuming 'dat' has been prepared and 'gamm_rt_on_mat' is the fitted model for 'RT'
...
predicted_RT <- predict(gamm_rt_on_mat$gam, newdata = data.frame(mature = maturation_ages), type = "response")

# Create a data frame to save
data_to_save_RT <- data.frame(mature = maturation_ages, predicted_RT = predicted_RT)

# Save the data frame to a CSV file
write.csv(data_to_save_RT, "predicted_RT_vs_mature.csv", row.names = FALSE)
